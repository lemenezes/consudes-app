...
Deno.serve(async (req: Request) => {
  ...
  const html = `
    <div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#1e293b">
      <div style="background:#0A3D62;border-radius:16px;padding:24px;margin-bottom:24px;text-align:center">
        <h1 style="margin:0;font-size:20px;font-weight:700;color:#fff">CONSUDES — Conselho de Entidades</h1>
      </div>

      <h2 style="margin:0 0 12px;font-size:18px;font-weight:700">Solicitação de acesso não aprovada</h2>

      <p style="margin:0 0 16px;color:#64748b">
        Olá, <strong>${request.full_name}</strong>. Infelizmente sua solicitação de acesso ao CONSUDES
      ...
      <p style="margin:0;font-size:11px;color:#94a3b8">CONSUDES — Conselho de Entidades</p>
    </div>
  `;
  ...
  const resendRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: FROM_EMAIL,
      to: request.email,
      subject: 'Sua solicitação de acesso ao CONSUDES',
      html,
    }),
  });
