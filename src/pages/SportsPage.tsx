...
import { Shield, CalendarDays, ListChecks, ArrowRight } from "lucide-react";
import { useLanguage } from "../context/LanguageContext";
import PageShell from "../components/PageShell";
import { useSEO } from "../hooks/useSEO";

const MODALITIES_DESC: Record<string, string> = {
  es: "Conoce las modalidades deportivas practicadas por nuestros atletas.",
  pt: "Conheça as modalidades esportivas praticadas por nossos atletas.",
  en: "Learn about the sports disciplines practiced by our athletes."
};
  ...
  const cards = [
    {
      ...
      desc: t.interclubsPage.heroSubtitle,
      Icon: Shield
    },
    {
      to: "/calendario",
      label: t.nav.calendar,
      desc: t.calendarPage.subtitle,
      Icon: CalendarDays
    },
    {
      to: "/esportes/modalidades",
      label: t.nav.modalities,
      desc: MODALITIES_DESC[lang],
      Icon: ListChecks
    }
  ];
            ...
            {cards.map(({ to, label, desc, Icon }) => (
              <Link
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  ...
                  <p className="text-[#1F2937]/55 dark:text-white/40 text-[12px] leading-relaxed">
                    {desc}
                  </p>
                </div>
                <ArrowRight
                  size={14}
                  className="flex-shrink-0 text-[#D9A441] opacity-0 group-hover:opacity-100 mt-1 transition-opacity"
                />
              </Link>
            ))}
          </div>
