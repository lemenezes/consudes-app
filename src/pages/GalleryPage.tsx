...
function AlbumCover({
  ...
  if (!src || failed) {
    return (
      <div
        className={`w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-consudes-blue/20 to-consudes-navy/40 dark:from-consudes-blue/25 dark:to-consudes-dark/70 ${className}`}>
        <svg
          className="w-10 h-10 text-white/25 mb-2"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.2}
          stroke="currentColor">
    ...
    <Link
      ...
      <div
        ...
        <div className="absolute top-3 left-3 flex gap-1.5">
          {album.tier === "T1" && (
            <span
              className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full ${tierColors.T1}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <h3
            className={`text-white leading-tight tracking-tight font-['Cormorant_Garamond'] ${featured ? "text-xl sm:text-3xl font-bold" : "text-base sm:text-xl font-semibold"}`}>
            {album.title}
          </h3>
...
export default function GalleryPage() {
  ...
  const availableYears = [
    ...new Set(albums.map(a => a.year).filter(Boolean))
  ].sort() as number[];
  const availableCountries = [
    ...new Set(albums.map(a => a.country).filter(Boolean))
  ].sort() as string[];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:hidden flex justify-end mb-2">
            <button
              ...
              {activeCount > 0 && (
                <span className="ml-1 text-xs font-bold text-consudes-gold">
                  ({activeCount})
                </span>
              )}
            </button>
              ...
              <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-consudes-gold/10 border border-consudes-gold/30 text-consudes-gold text-base font-semibold shadow-sm">
                <svg
                  ...
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.462 2.063-2.92A17.944 17.944 0 0012 4a17.944 17.944 0 00-7.98 12.08c-.439 1.458.523 2.92 2.062 2.92z"
                  />
                </svg>
