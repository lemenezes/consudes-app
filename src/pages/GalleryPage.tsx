...
function AlbumCover({ album, className = '' }: { album: GalleryAlbum; className?: string }) {
...
function AlbumCard({ album, featured = false, t }: {
  album: GalleryAlbum;
  featured?: boolean;
      ...
      <div className={`relative overflow-hidden ${featured ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          {featured && album.description && (
            <p className="text-white/60 text-sm mt-2 line-clamp-2 hidden sm:block">{album.description}</p>
          )}
        </div>
      </div>
    </Link>
      ...
      <section className="bg-white dark:bg-consudes-dark-body py-12 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {!activeCategory && featured && (
            <div className="mb-6">
              <AlbumCard album={featured} featured t={t} />
            </div>
          )}
          ...
          {gridAlbums.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {gridAlbums.map((album) => (
                <AlbumCard key={album.slug} album={album} t={t} />
              ))}
            </div>
          ) : (
