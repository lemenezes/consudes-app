...
type LocalizedKey = keyof Pick<NewsFormData,
...
function getLocalizedValue(form: NewsFormData, field: LocalizedField, lang: Lang): string {
...
export default function AdminNewsFormPage() {
  ...
  const [form, setForm] = useState<NewsFormData>(withLegacyCompatibility(EMPTY));
  const [initialForm, setInitialForm] = useState<NewsFormData>(withLegacyCompatibility(EMPTY));
  ...
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<LocalizedKey, string>>>({});
  const [slugEdited, setSlugEdited] = useState(false);

  // Snapshot original carregado do banco para detectar imagens novas/removidas.
  const originalContentSnapshotRef = useRef<Record<Lang, string>>({ es: '', pt: '', en: '' });
  ...
  useEffect(() => {
    ...
    getNewsById(id).then(({ data, error: loadError }) => {
      ...
      const loaded = withLegacyCompatibility({
        ...
        excerpt_pt: data.excerpt_pt ?? (data.lang === 'pt' ? (data.excerpt ?? '') : ''),
        excerpt_es: data.excerpt_es ?? (data.lang === 'es' ? (data.excerpt ?? '') : ''),
        excerpt_en: data.excerpt_en ?? (data.lang === 'en' ? (data.excerpt ?? '') : ''),
        content_pt: data.content_pt ?? (data.lang === 'pt' ? (data.content ?? '') : ''),
        content_es: data.content_es ?? (data.lang === 'es' ? (data.content ?? '') : ''),
        content_en: data.content_en ?? (data.lang === 'en' ? (data.content ?? '') : ''),
        title: data.title,
  ...
  const updateLocalizedField = (field: LocalizedField, lang: Lang, value: string) => {
  ...
  const focusFirstInvalidOriginalField = (errors: Partial<Record<LocalizedKey, string>>) => {
    if (errors[originalTitleKey]) {
      originalTitleRef.current?.focus();
      originalTitleRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }

    if (errors[originalExcerptKey]) {
      originalExcerptRef.current?.focus();
      originalExcerptRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }

    if (errors[originalContentKey]) {
      originalContentAnchorRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  };
  ...
  const validate = (): boolean => {
    ...
    if (Object.keys(nextFieldErrors).length > 0) {
      ...
      requestAnimationFrame(() => focusFirstInvalidOriginalField(nextFieldErrors));
      return false;
    }
  ...
  const handleGenerateTranslations = async () => {
    ...
    if (!sourceTitle || !sourceExcerpt || !sourceContent) {
      setError('Preencha titulo, resumo e conteudo no idioma original antes de gerar traducoes.');
      setActiveLang(sourceLang);
      requestAnimationFrame(() => focusFirstInvalidOriginalField({
        [originalTitleKey]: !sourceTitle ? REQUIRED_MESSAGE.title : undefined,
        [originalExcerptKey]: !sourceExcerpt ? REQUIRED_MESSAGE.excerpt : undefined,
        [originalContentKey]: !sourceContent ? REQUIRED_MESSAGE.content : undefined,
      }));
      return;
    }

    const targets = LANG_TABS.map((item) => item.code).filter((code) => code !== sourceLang);
    const hasSomethingToGenerate = targets.some((target) => {
      return !getLocalizedValue(form, 'title', target).trim()
        || !getLocalizedValue(form, 'excerpt', target).trim()
        || !getLocalizedValue(form, 'content', target).trim();
    });

    if (!hasSomethingToGenerate) {
      showToast('Todos os idiomas ja possuem conteudo. Nada para gerar.', 'success');
      return;
    }
    ...
    try {
      ...
      const patches = await Promise.all(
        targets.map(async (target) => {
          ...
          if (!currentTitle.trim()) {
            patch[fieldKey('title', target)] = await translatePlain(sourceTitle, sourceLang, target);
            generated += 1;
          } else {
            preserved += 1;
          }
          ...
          if (!currentExcerpt.trim()) {
            patch[fieldKey('excerpt', target)] = await translatePlain(sourceExcerpt, sourceLang, target);
            generated += 1;
          } else {
            preserved += 1;
          }
          ...
          if (!currentContent.trim()) {
            patch[fieldKey('content', target)] = await translateHTML(sourceContent, sourceLang, target);
            generated += 1;
          } else {
            preserved += 1;
          }
      ...
      if (generated === 0) {
        ...
        showToast(`Traducoes geradas (${generated} campos). ${preserved} campo(s) existente(s) foram mantidos.`, 'success');
      } else {
        showToast(`Traducoes geradas com sucesso (${generated} campos).`, 'success');
      }
    } catch {
      setError('Falha ao gerar traducoes. Verifique sua conexao e tente novamente.');
  ...
  const handleCancel = async () => {
    const toDelete = mergeUnique(
      getNewlyAddedInlineUrls(originalContentSnapshotRef.current.es, form.content_es),
      getNewlyAddedInlineUrls(originalContentSnapshotRef.current.pt, form.content_pt),
      getNewlyAddedInlineUrls(originalContentSnapshotRef.current.en, form.content_en),
    );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isEditing && id) {
      const { data, error: saveError } = await updateNews(id, payload, previousStatus);
      if (saveError) {
        setError(saveError);
        setSaving(false);
        return;
      ...
      await log({ action, entity_type: 'news', entity_id: data?.id, entity_title: payload.title });

      const orphaned = mergeUnique(
        getOrphanedInlineUrls(originalContentSnapshotRef.current.es, payload.content_es),
        getOrphanedInlineUrls(originalContentSnapshotRef.current.pt, payload.content_pt),
        getOrphanedInlineUrls(originalContentSnapshotRef.current.en, payload.content_en),
      );
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <h1 className="text-2xl font-['Cormorant_Garamond'] font-semibold text-[#1F2937] leading-none">
            {originalTitle || <span className="text-gray-300">{t.admin.noTitle}</span>}
          </h1>
        </div>
      </div>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-3 py-4 sm:px-6 sm:py-5 space-y-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <label htmlFor="original_language" className="block text-[11px] font-bold uppercase tracking-widest text-gray-400">
                ...
                <select
                  ...
                  onChange={(e) => handleOriginalLanguageChange(e.target.value as Lang)}
              ...
              <div className="flex flex-wrap gap-2">
                {LANG_TABS.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      {tab.label}{isOriginal ? ' (original)' : ''}{hasData ? '' : ' *'}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-gray-500">
                  Edite qualquer idioma manualmente. Gerar traducoes so preenche campos vazios.
                </p>
                <button
                  type="button"
                ...
                >
                  {translating ? 'Gerando traducoes...' : 'Gerar traducoes'}
                </button>
              </div>
            </div>

            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-3 py-4 sm:px-6 sm:py-5">
              <label htmlFor={`title_${activeLang}`} className="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-3">
                {t.admin.titleLabel} {activeLang === form.original_language ? <span className="text-red-400">*</span> : null}
              </label>
              <input
                ...
                ref={activeLang === form.original_language ? originalTitleRef : undefined}
              ...
              {fieldErrors[activeTitleKey] && (
                <p className="text-xs text-red-600 mt-2">{fieldErrors[activeTitleKey]}</p>
              )}
            </div>

            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-3 py-4 sm:px-6 sm:py-5">
              <label htmlFor={`excerpt_${activeLang}`} className="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-3">
                Resumo {activeLang === form.original_language ? <span className="text-red-400">*</span> : null}
              </label>
              <textarea
                ...
                ref={activeLang === form.original_language ? originalExcerptRef : undefined}
              ...
              {fieldErrors[activeExcerptKey] && (
                <p className="text-xs text-red-600 mt-2">{fieldErrors[activeExcerptKey]}</p>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.07)] overflow-hidden">
              <div className="px-3 py-2 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between sm:px-6 sm:py-3.5">
                <span className="text-[11px] font-bold uppercase tracking-widest text-gray-400">
                  {t.admin.contentLabel} {activeLang === form.original_language ? <span className="text-red-400">*</span> : null}
                </span>
                <span className="text-[10px] text-gray-300 font-medium">Rich Text</span>
              </div>
              <div
                className="p-2 sm:p-4"
                ref={activeLang === form.original_language ? originalContentAnchorRef : undefined}
              >
                <RichTextEditor
                  value={activeContent}
                  onChange={(html) => updateLocalizedField('content', activeLang, html)}
                  placeholder={t.admin.contentPlaceholder}
                />
              </div>
              {fieldErrors[activeContentKey] && (
                <p className="text-xs text-red-600 px-3 pb-3 sm:px-6 sm:pb-4">{fieldErrors[activeContentKey]}</p>
              )}
            </div>
            ...
            <div className="pt-2 pb-8 space-y-3 px-1 sm:px-0">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                >
                  {saving ? 'Salvando...' : isEditing ? t.admin.saveChanges : t.admin.publish}
                </button>
              </div>
            </div>
          </div>
