...
import type { CalendarEventRow, PublishStatus } from '../../lib/database.types';
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400 text-sm">{ac.noEvents}</div>
      ) : (
        <>
          {/* ── Mobile: cards ─────────────────────────────────────────────── */}
              ...
              <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col gap-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-[#1F2937] leading-snug line-clamp-2">{item.title}</p>
