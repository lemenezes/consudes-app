...
export default function TransparencyPage() {
  ...
  const grouped = filtered.reduce<Record<number, ReportPublicItem[]>>(
    (acc, r) => {
      ...
      <PageShell
        ...
        <section className="bg-white dark:bg-consudes-dark-body py-20">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-10">
              <p
                className="text-[11px] font-bold tracking-[0.5em] uppercase text-consudes-gold mb-3"
                aria-hidden="true">
                CONSUDES
              </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-8">
              <div className="relative flex-1">
                <svg
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#1F2937]/35 dark:text-white/30"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={1.8}
                  stroke="currentColor"
              ...
              <select
                value={filterCat}
                onChange={e =>
                  setFilterCat(e.target.value as ReportCategory | "")
                }
                ...
                aria-label={tp.filterCategoryLabel}>
                <option value="">{tp.filterAllCategories}</option>
                {REPORT_CATEGORIES.map(c => (
                  <option key={c.value} value={c.value}>
                    {tp.categories[c.value]}
                  </option>
                ))}
              </select>
              <select
                ...
                className="px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-[#1F2937] dark:text-white focus:outline-none focus:ring-2 focus:ring-[#003B73]/30"
                aria-label={tp.filterYearLabel}>
                <option value="">{tp.filterAllYears}</option>
                {years.map(y => (
                ...
                {sortedYears.map(year => (
                  <div key={year}>
                    <div className="flex items-center gap-4 mb-4">
                      <span
                        className="font-['Cormorant_Garamond'] text-4xl font-bold text-[#003B73]/10 dark:text-white/10 leading-none select-none"
                        aria-hidden="true">
                        {year}
                      </span>
                      <div
                        className="flex-1 h-px bg-gradient-to-r from-[#003B73]/10 via-[#D9A441]/25 to-transparent dark:from-white/8 dark:via-[#D9A441]/15"
                        aria-hidden="true"
                      />
                    </div>

                    <ul className="space-y-3" aria-label={String(year)}>
                      {grouped[year].map(doc => (
                        <li
                          ...
                          <div
                            className="hidden sm:flex items-center justify-center w-10 h-10 rounded-lg bg-[#003B73]/6 dark:bg-white/8 text-[#003B73] dark:text-white/70 shrink-0 group-hover:bg-[#D9A441]/10 transition-colors"
                            aria-hidden="true">
                            <IconFile />
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase ${CATEGORY_COLORS[doc.category]}`}>
                                {tp.categories[doc.category]}
                              </span>
                              {doc.doc_date && (
                                <time
                                  dateTime={doc.doc_date}
                                  className="text-[11px] text-[#1F2937]/45 dark:text-white/35">
                                  {new Date(
                                    doc.doc_date + "T12:00:00"
                                  ).toLocaleDateString(dateLocale[lang], {
                                    day: "2-digit",
                                    month: "short",
                                    year: "numeric"
                                  })}
                          ...
                          {doc.file_url ? (
                            <div className="shrink-0 flex items-center gap-2">
                              <button
                                ...
                                aria-label={`${tp.viewBtn} ${doc.title}`}
                              ...
                              <a
                                href={doc.file_url}
                                download
                                aria-label={`${tp.downloadBtn} ${doc.title}`}
