    ...
    <div
      ...
      {options.map(opt => (
        <button
          ...
          className={`px-3 py-1.5 text-sm font-semibold rounded-md border border-transparent transition-colors
            ${
              value === opt.value
                ? "bg-consudes-blue text-white shadow-sm"
                : "bg-transparent text-slate-700 dark:text-slate-200 hover:bg-consudes-blue/10"
            }
            focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-consudes-blue/50/80 focus-visible:ring-offset-2`}>
          {opt.label}
        </button>
      ))}
    </div>
