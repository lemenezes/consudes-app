...
export default function NewsCard({ news, compact = false }: NewsCardProps) {
  ...
  return (
    <Link
      to={`/noticias/${slug}`}
      className={`
        group flex flex-col bg-white dark:bg-consudes-dark rounded-2xl overflow-hidden
    ...
    >
      ...
      <div className={`relative overflow-hidden bg-consudes-surface dark:bg-consudes-dark-body shrink-0 ${compact ? 'h-40' : 'h-48'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-consudes-gold/0 via-consudes-gold/60 to-consudes-gold/0 opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
      ...
      <div className="flex flex-col flex-1 p-5 gap-2">
        ...
        {/* Título */}
        <h3 className="font-['Cormorant_Garamond'] text-lg font-semibold text-consudes-blue-text dark:text-white leading-snug group-hover:text-consudes-blue-mid dark:group-hover:text-white transition-colors line-clamp-2">
          {title}
        </h3>
        ...
        {/* Leer más */}
        <div className="flex items-center gap-1 text-consudes-blue-mid dark:text-consudes-gold text-xs font-semibold mt-1 group-hover:gap-2 transition-all">
